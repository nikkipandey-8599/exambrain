...
import { Star, Send, ThumbsUp, MessageSquare, X, ChevronDown, ChevronUp } from 'lucide-react'
...
// ── Review Card ───────────────────────────────────────
function ReviewCard({ review, onThumbsUp }) {
    ...
    <div className="animate-slideUp" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{
            ...
            background: `hsl(${review.name?.charCodeAt(0) * 17 % 360}, 60%, 45%)`,
      ...
      <button
        ...
        style={{
          ...
          fontSize: '0.75rem', color: liked ? 'var(--success)' : 'var(--text-muted)',
          transition: 'all 0.2s'
        }}
      >
...
export default function ReviewSection({ user }) {
  ...
  async function handleSubmit() {
    ...
    if (!message.trim() || message.trim().length < 10) { showToast.error('Please write at least 10 characters'); return }
    ...
    <div style={{ padding: '1.25rem', paddingBottom: '5rem' }}>
      ...
      {showForm && (
        <div className="animate-slideDown card" style={{ marginBottom: '1.25rem' }}>
          ...
          <textarea
            className="input-field"
            rows={4}
            placeholder="What do you think? What's working well? What would you like to see improved?"
            value={message}
            onChange={e => setMessage(e.target.value)}
            maxLength={500}
            style={{ marginBottom: 6, fontSize: '0.88rem', resize: 'vertical' }}
          />
          ...
          <button
            ...
            disabled={submitting || !stars || message.trim().length < 10}
            className="btn-primary"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
          >
            {submitting ? (
              <>
                <span className="animate-spin" style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'white', borderRadius: '50%', display: 'inline-block' }} />
                Submitting…
              </>
            ) : (
              ...
              {dist.map(({ stars: s, count }) => (
                <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  ...
                  <div style={{ flex: 1, height: 6, background: 'var(--bg-secondary)', borderRadius: 99, overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 99, background: 'var(--warning)',
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '2.5rem 1rem', background: 'var(--bg-secondary)', borderRadius: 16, border: '1px solid var(--border)' }}>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>Your feedback helps make ExamBrain better for everyone.</p>
        </div>
      ) : (
        <>
          {displayed.map(r => (
            <ReviewCard key={r.id} review={r} onThumbsUp={async (id, cur) => {
